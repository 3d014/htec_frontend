...
const [newTotalBudget, setNewTotalBudget] = useState<number | string>('');
    ...
    const handleSubmitAllowedBudget = async () => {
        try {
            const totalBudget=allowedBudget
    ...
    const handleConfirmSubmit = () => {
        if(allowedBudget==null||Number.isNaN(allowedBudget)||allowedBudget==0){
            return toast('Insert allowed budget')
        }
    ...
    const handleAllowedBudgetChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            <Box sx={{ marginTop: '10px', display: 'flex', flexDirection: 'row', gap: '20px', padding: '20px', justifyContent: 'space-around', flexWrap: 'wrap' }}>
                ...
                <GenericModal isOpen={isAddingBudget} onClose={() => { setIsAddingBudget(false) }}>
                    ...
                    <Box sx={{ marginTop: '20px', display: 'flex', justifyContent: 'space-evenly' }}>
                        <Typography sx={{ color: 'white', fontWeight: '20px', backgroundColor: '#32675B', width: '200px', padding: '10px', borderRadius: '5px' }}>Enter allowed budget value in textfield </Typography>
                        <TextField sx={{ justifyContent: 'center', backgroundColor: 'white' }}
                            ...
                            value={allowedBudget}
                            onChange={handleAllowedBudgetChange}
                        ></TextField>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '50px' }}>
                        <Typography sx={{ width: '150px', backgroundColor: '#32675B', color: 'white', textAlign: 'center', borderRadius: '5px', cursor: 'pointer' }} onClick={handleConfirmSubmit}>Submit</Typography>
                    </Box>
                </GenericModal>
                <GenericModal isOpen={isConfirmationOpen} onClose={handleCancelSubmit}>
                    <Typography sx={{ color: 'black', fontSize: '24px', fontWeight: 'bold' }}>Confirm Submission</Typography>
                    <Typography sx={{ color: 'black' }}>Are you sure you want to submit the allowed budget for {selectedMonth} / {selectedYear}?</Typography>
                    <Box sx={{ marginTop: '20px', display: 'flex', justifyContent: 'space-evenly' }}>
                        <Button variant="contained" sx={{ backgroundColor: '#32675B' }} onClick={handleSubmitAllowedBudget}>Yes</Button>
                        <Button variant="contained" sx={{ backgroundColor: '#D32F2F' }} onClick={handleCancelSubmit}>No</Button>
                    </Box>
                </GenericModal>
            </Box>
            <Container>
